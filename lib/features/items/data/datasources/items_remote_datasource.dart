...
class ItemsRemoteDatasource extends Services<Item> {

  final String? url = "${Config.urlApi}item";

  Future<List<Item>?> getInflow(DateTime start, DateTime finish) async {
  ...
  Future<List<Item>?> getOutflow(DateTime start, DateTime finish) async {
    User? user = await AuthRemoteDatasource.getAuthUser();
    DateFormat dateFormat = DateFormat("yyyyMMdd");
    Response response = await Requests.get(
      "$url/outflow",
  ...
  Future save(Item data) async {
    ...
    if(response.statusCode == 201){
      return Item.toObject(response.json());
