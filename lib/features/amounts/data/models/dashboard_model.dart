...
class DashboardModel extends Dashboard{
  ...
  factory DashboardModel.fromJson(Map<String, dynamic> data){
    ...
    List<AmountModel>? amountInflowCategory =  List.of(data['amountInflowCategory']).map((el)=>AmountModel.fromJson(el)).toList();
    List<AmountModel>? amountOutflowCategory = List.of(data['amountOutflowCategory']).map((el)=>AmountModel.fromJson(el)).toList();
    List<AmountModel>? amountInflowCard = List.of(data['amountInflowCard']).map((el)=>AmountModel.fromJson(el)).toList();
    List<AmountModel>? amountOutflowCard = List.of(data['amountOutflowCard']).map((el)=>AmountModel.fromJson(el)).toList();
    
    List<TransferBankAmountModel>? amountInflowTransferBank = List.of(data['amountInflowTransferBank']).map((el)=>TransferBankAmountModel.fromJson(el)).toList();

    
    List<TransferBankAmountModel>? amountOutflowTransferBank = List.of(data['amountOutflowTransferBank']).map((el)=>TransferBankAmountModel.fromJson(el)).toList();
