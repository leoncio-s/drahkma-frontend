...
class InflowState extends State<InflowView> {
  static final AppNotifier _appNotifier = appNotifier;
  DateTime startDate = _appNotifier.dateTimeRange.start;
  DateTime finishDate = _appNotifier.dateTimeRange.end;
  List<ItemDto>? _items;
  ...
  void _getData() async {
    ...
    ItemsService().getInflow(startDate, finishDate).then((data) {
      setState(() {
        _items = data;
        _message = null;
      });
          ...
          onPressed: () {
            Navigator.of(context)
                .push(MaterialPageRoute(
                    builder: (context) => const ItemsForm(expense: false)))
                .then((data) {
              if (data is ItemDto) {
                // setState(() {});
                _getData();
                ...
                children: [
                  TextButton.icon(
                      ...
                      onPressed: () async {
                        ...
                        // DateTimeRange? date = await _appNotifier.selectDateRange(context);
                        if (date != null) {
                          setState(() {
